...
description: Ovládání zobrazení mřížky v listu aplikace Excel pomocí Aspose.Cells pro .NET.
type: docs
weight: 30
url: /cs/net/excel-display-settings-csharp-tutorials/display-and-hide-gridlines-of-worksheet/
---
V tomto tutoriálu vám ukážeme, jak zobrazit a skrýt mřížku v listu aplikace Excel pomocí zdrojového kódu C# s Aspose.Cells for .NET. Chcete-li dosáhnout požadovaného výsledku, postupujte podle níže uvedených kroků.
...
## Krok 3: Přejděte na první list a skryjte mřížku

 Přístup k prvnímu listu v souboru aplikace Excel pomocí`Worksheets` vlastnictvím`Workbook` objekt. Poté použijte`IsGridlinesVisible` vlastnictvím`Worksheet` objekt pro skrytí čar mřížky.
...
### Ukázkový zdrojový kód pro zobrazení a skrytí mřížky listu pomocí Aspose.Cells pro .NET 
...
Tento podrobný průvodce vám ukázal, jak zobrazit a skrýt mřížku v tabulce aplikace Excel pomocí Aspose.Cells for .NET. Pomocí dodaného zdrojového kódu C# můžete snadno přizpůsobit zobrazení mřížky v souborech aplikace Excel.
...
#### Jak mohu zobrazit nebo skrýt mřížku v tabulce aplikace Excel pomocí Aspose.Cells pro .NET?

 Můžete použít`IsGridlinesVisible` vlastnictvím`Worksheet` objekt pro zobrazení nebo skrytí čar mřížky. Nastavte na`true` ukázat jim a`false` schovat je.
