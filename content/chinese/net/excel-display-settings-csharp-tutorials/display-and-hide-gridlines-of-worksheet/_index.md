...
---
在本教程中，我们将向您展示如何使用 C# 源代码和 Aspose.Cells for .NET 在 Excel 工作表中显示和隐藏网格线。请按照以下步骤操作以获得所需的结果。
...
使用以下命令访问 Excel 文件中的第一个工作表`Worksheets`的财产`Workbook`目的。然后使用`IsGridlinesVisible`的财产`Worksheet`隐藏网格线的对象。
...
您可以使用`IsGridlinesVisible`的财产`Worksheet`显示或隐藏网格线的对象。将其设置为`true`向他们展示并`false`隐藏它们。
