...
description: Apprenez à manipuler facilement des fichiers Excel et à personnaliser les options d'impression à l'aide d'Aspose.Cells pour .NET.
type: docs
weight: 170
url: /fr/net/excel-page-setup/set-excel-print-title/
---
Dans ce guide, nous vous expliquerons comment définir des titres d'impression dans une feuille de calcul Excel à l'aide d'Aspose.Cells pour .NET. Suivez les étapes ci-dessous pour accomplir cette tâche.
...
## Étape 6 : Définir les colonnes de titre
...
## Étape 8 : Enregistrer le classeur Excel

 Pour enregistrer le classeur Excel avec les titres d'impression définis, utilisez la`Save` méthode de l'objet Workbook :
...
### Exemple de code source pour définir le titre d'impression Excel à l'aide de Aspose.Cells pour .NET 
```csharp
...
// Définir les numéros de colonne A et B comme colonnes de titre
pageSetup.PrintTitleColumns = "$A:$B";
...
Félicitation ! Vous avez appris à définir des titres d'impression dans une feuille de calcul Excel à l'aide d'Aspose.Cells pour .NET. Les titres d'impression vous permettent d'afficher des lignes et des colonnes spécifiques sur chaque page imprimée, ce qui facilite la lecture et la référence des données.

### FAQ

#### 1. Puis-je définir des titres d'impression pour des colonnes spécifiques dans Excel ?

 Oui, avec Aspose.Cells pour .NET, vous pouvez définir des colonnes spécifiques comme titres d'impression à l'aide du`PrintTitleColumns` propriété de la`PageSetup` objet.

#### 2. Est-il possible de définir à la fois des titres de colonne et de ligne d'impression ?

 Oui, vous pouvez définir à la fois les titres des colonnes et des lignes à l'aide de la`PrintTitleColumns` et`PrintTitleRows` propriétés de la`PageSetup` objet.
