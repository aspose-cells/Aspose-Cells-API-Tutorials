...
これらの名前空間をインポートすることで、Aspose.Cells によって提供されるクラスとメソッドを利用して Excel ファイルとグラフィックスを操作できるようになります。
...
ここで、ワークシートにサンプル データを挿入してみましょう。このデータは後でグラフに表示されます。
...
ここでは、ワークシートの A 列と B 列にデータを配置しています。このデータはグラフのデータセットとして機能します。簡単な質問: セルに数字が入力されるのを見るのは満足感がありませんか?
...
指定されたセル内に配置された縦棒グラフを追加します。このグラフは、列内のデータを視覚化するのに役立ち、値の比較を容易にします。
