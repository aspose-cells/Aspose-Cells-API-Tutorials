...
ここで、Excel スプレッドシートで範囲を書式設定するプロセスを、明確で管理しやすい手順に分解してみましょう。
...
いよいよ、楽しい部分、つまりスタイル設定に入ります。範囲に適用するスタイル オブジェクトを作成しましょう。
...
説明: 範囲を明るい黄色で塗りつぶします。単色のパターンにより塗りつぶしの一貫性が確保され、データが太字の赤いフォントに対して目立つようになります。
...
```csharp
workbook.Save(dataDir + "outputFormatRanges1.xlsx"); //指定されたディレクトリにワークブックを保存します
```

説明: ここでは、先ほど設定したディレクトリに「outputFormatRanges1.xlsx」として作業内容を保存します。フォーマットされた Excel シートが作成された瞬間をぜひ味わってください。
