...
この重要な手順により、ワークシートの改ページ プレビュー モードが有効になります。後でファイルを開いたときに、これがレイアウトと印刷書式にどのように影響するかを確認できます。
...
最後に、リソースをクリーンアップすることは良い習慣です。ファイル ストリームを閉じると、それに関連付けられているすべてのリソースが解放され、メモリ リークが防止されます。
