...
## 結論
これで完了です。Aspose.Cells for .NET を使用して Excel ブックを HTML に保存する際に、ダウンレベルの公開コメントを無効にする方法を学習しました。この機能を使用すると、ブックを HTML としてレンダリングする方法を制御し、不要なコンテンツが公開されないようにすることができます。Web アプリを開発している場合でも、単にクリーンな HTML 出力が必要な場合でも、この方法により、ブックの変換が正確かつ安全になります。
