...
```
 ...
 Здесь мы создаем новый экземпляр`Workbook` class и загрузите в него наш файл Excel. Представьте себе рабочую книгу как цифровой блокнот, содержащий все ваши листы и диаграммы. Параметр, который мы передаем, — это полный путь к нашему файлу Excel, поэтому убедитесь, что он включает имя файла!
