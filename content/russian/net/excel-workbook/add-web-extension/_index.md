...
```
 Здесь,`RunExamples.Get_OutputDirectory()` — это метод, который извлекает путь к выходному каталогу. Вы можете изменить его, чтобы указать на любое место в вашей системе.
