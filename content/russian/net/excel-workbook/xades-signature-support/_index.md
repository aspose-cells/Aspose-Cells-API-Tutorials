...
```
 ...
 В этом фрагменте кода`RunExamples.Get_SourceDirectory()` и`RunExamples.Get_OutputDirectory()` — это методы, которые возвращают пути к исходному и выходному каталогам соответственно. Обязательно замените их на фактические пути, если вы не используете эти методы.
