...
description: 使用 Aspose.Cells for .NET 在 Excel 工作表中显示或隐藏行和列标题。
type: docs
weight: 40
url: /zh/net/excel-display-settings-csharp-tutorials/display-and-hide-row-column-headers-of-worksheet/
---
在本教程中，我们将向您展示如何使用 C# 源代码和 Aspose.Cells for .NET 显示或隐藏 Excel 工作表的行和列标题。请按照以下步骤获得所需的结果。
...
## 第 3 步：转到第一个工作表并隐藏行和列标题

使用访问 Excel 文件中的第一个工作表`Worksheets`的财产`Workbook`目的。然后使用`IsRowColumnHeadersVisible`的财产`Worksheet`对象以隐藏行和列标题。
...
本分步指南向您展示了如何使用 Aspose.Cells for .NET 在 Excel 电子表格中显示或隐藏行和列标题。使用提供的 C# 源代码，您可以轻松自定义 Excel 文件中标题的显示。
...
您可以使用`IsRowColumnHeadersVisible`的财产`Worksheet`显示或隐藏行和列标题的对象。设置为`true`向他们展示并`false`隐藏它们。
