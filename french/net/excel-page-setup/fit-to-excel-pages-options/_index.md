...
description: Apprenez à ajuster automatiquement des pages dans une feuille de calcul Excel avec Aspose.Cells pour .NET.
type: docs
weight: 30
url: /fr/net/excel-page-setup/fit-to-excel-pages-options/
---
Dans cet article, nous vous expliquerons étape par étape le code source C# suivant : Options d'adaptation aux pages Excel à l'aide d'Aspose.Cells pour .NET. Nous utiliserons la bibliothèque Aspose.Cells pour .NET pour effectuer cette opération. Suivez les étapes ci-dessous pour configurer l'ajustement aux pages dans Excel.
...
## Étape 3 : Définir l'ajustement aux pages
 Dans cette étape, nous allons configurer l'ajustement aux pages de la feuille de calcul. Nous utiliserons le`FitToPagesTall` et`FitToPagesWide` propriétés de la`PageSetup` objet pour spécifier le nombre de pages souhaité pour la hauteur et la largeur de la feuille de calcul. Voici le code pour cela :
...
### Exemple de code source pour les options d'ajustement aux pages Excel à l'aide d'Aspose.Cells pour .NET 
```csharp
